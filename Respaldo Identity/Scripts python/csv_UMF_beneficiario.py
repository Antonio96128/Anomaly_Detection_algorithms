...
def crearUMF(cID, cfullName, cBP):
   ...
   return uEntity(
       dCode("BENEFICIARIOSRC"),
       dAction("A"),
       dAcct(str(cID)+ str(cBP)),
       dRef("Beneficiario_" + str(cID)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_BENEFICIARIO"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'E_Beneficiario.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
	    try:
      		doc = crearUMF(row[0],row[1],row[3])
                mensaje = etree.tostring(doc)
# 		print(mensaje,type(mensaje))
                with open(path2 + 'E_Beneficiario.UMF', 'a+') as archivo:
