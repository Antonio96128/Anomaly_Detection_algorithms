...
def crearUMF(cID, ccorreo):
   ccorreo = ccorreo.strip()
   if ccorreo == "": 
       ccorreo = "NULO"

   return uEntity(
       dCode("CORREOELECTRONICOSRC"),
       dAction("A"),
       dAcct(str(cID)+ str(ccorreo)),
       dRef("CorreoElectronico_" + str(cID)),       
         roles(
          rCode("ROL_CORREO_ELECTRONICO"),
          rstat("C"),
       ),      
        number(
          nuType("CORREOELECTRONICO"),
          nValue(str(ccorreo)),
       ),
   )
...
if __name__ == "__main__":
    with open(path + 'E_Correo_Electronico.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'E_Correo_Electronico.UMF', 'a+') as archivo:
