...
def crearUMF(cfullName, cRFC, cBP):
   ...
   return uEntity(
       dCode("REGLA5TITULARSRC"),
       dAction("A"),
       dAcct(str(cBP)+ str(cfullName)),
       dRef("Regla5_Titular" + str(cBP)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REGLA_5_TITULAR"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Titular.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Titular.UMF', 'a+') as archivo:
