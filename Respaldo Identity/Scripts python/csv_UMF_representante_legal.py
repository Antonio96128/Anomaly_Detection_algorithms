...
def crearUMF(cID, cfullName, cRFC, cBP):
   ...
   return uEntity(
       dCode("REPRESENTANTELEGALSRC"),
       dAction("A"),
       dAcct(str(cID)+ str(cBP)),
       dRef("RepLegal_" + str(cID)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REPRESENTANTE_LEGAL"),
          rstat("C"),
       ),      
        number(
          nuType("RFC"),
...
if __name__ == "__main__":
    with open(path + 'E_Representante_Legal.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'E_Representante_Legal.UMF', 'a+') as archivo:
