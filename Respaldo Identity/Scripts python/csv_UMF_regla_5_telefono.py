...
def crearUMF(cPH):
   ...
   return uEntity(
       dCode("REGLA5TELEFONOSRC"),
       dAction("A"),
       dAcct(str(cPH)),
       dRef("Regla5_Telefono" + str(cPH)),       
         roles(
          rCode("ROL_REGLA_5_TELEFONO"),
          rstat("C"),
       ),      
        number(
          nuType("PH"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Telefono.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Telefono.UMF', 'a+') as archivo:
