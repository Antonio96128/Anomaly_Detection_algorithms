...
def crearUMF(cBP, cfullName):
   ...
   return uEntity(
       dCode("REGLA5ACCIONISTASRC"),
       dAction("A"),
       dAcct(str(cBP)+ str(cfullName)),
       dRef("Regla5_Accionista" + str(cBP)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REGLA_5_ACCIONISTA"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Accionista.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Accionista.UMF', 'a+') as archivo:
