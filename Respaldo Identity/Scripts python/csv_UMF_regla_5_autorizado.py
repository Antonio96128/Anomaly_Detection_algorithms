...
def crearUMF(cRFC, cBP, cfullName):
   ...
   return uEntity(
       dCode("REGLA5AUTORIZADOSRC"),
       dAction("A"),
       dAcct(str(cBP)+ str(cfullName)),
       dRef("Regla5_Autorizado" + str(cBP)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REGLA_5_AUTORIZADO"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Autorizado.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Autorizado.UMF', 'a+') as archivo:
