...
def crearUMF(cCorreo):
   cCorreo = cCorreo.strip()
   if cCorreo == "": 
       cCorreo = "NULO"

   return uEntity(
       dCode("REGLA5CORREOSRC"),
       dAction("A"),
       dAcct(str(cCorreo)),
       dRef("Regla5_Correo" + str(cCorreo)),       
         roles(
          rCode("ROL_REGLA_5_CORREO"),
          rstat("C"),
       ),      
        number(
          nuType("CORREO"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Correo_Electronico.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Correo_Electronico.UMF', 'a+') as archivo:
