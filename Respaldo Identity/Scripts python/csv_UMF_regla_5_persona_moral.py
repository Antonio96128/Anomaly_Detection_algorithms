...
def crearUMF(cfullName, cRFC, cBP):
   ...
   return uEntity(
       dCode("REGLA5PERSONAMORALSRC"),
       dAction("A"),
       dAcct(str(cBP)+ str(cfullName)),
       dRef("Regla5_Persona_Moral" + str(cBP)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REGLA_5_PERSONA_MORAL"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Persona_Moral.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Persona_Moral.UMF', 'a+') as archivo:
