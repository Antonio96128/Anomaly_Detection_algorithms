...
def crearUMF(cID, cphone):
   ...
   return uEntity(
       dCode("TELEFONOSRC"),
       dAction("A"),
       dAcct(str(cID)+ str(cphone)),
       dRef("Telefono_" + str(cID)),       
         roles(
          rCode("ROL_TELEFONO"),
          rstat("C"),
       ),      
...
if __name__ == "__main__":
    with open(path + 'E_Telefono.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'E_Telefono.UMF', 'a+') as archivo:
