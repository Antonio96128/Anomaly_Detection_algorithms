...
def crearUMF(cfullName, cRFC, cBP):
   cfullName, cBP, cRFC = cfullName.strip(), cBP.strip(), cRFC.strip()
   if cBP == "": 
       cBP = "NULO"
   if cfullName == "": 
       cfullName = "NULO"
   ...
   return uEntity(
       dCode("REGLA5REPRESENTANTELEGALSRC"),
       dAction("A"),
       dAcct(str(cBP)+ str(cfullName)),
       dRef("Regla5_Representante_Legal" + str(cBP)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REGLA_5_REPRESENTANTE_LEGAL"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Representante_Legal.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
	    try:
      		doc = crearUMF(row[0],row[1],row[2])
                mensaje = etree.tostring(doc)
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Representante_Legal.UMF', 'a+') as archivo:
