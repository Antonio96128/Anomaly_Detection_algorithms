...
def crearUMF(cfullName, cRFC, cBP):
   ...
   return uEntity(
       dCode("REGLA5COTITULARSRC"),
       dAction("A"),
       dAcct(str(cBP)+ str(cfullName)),
       dRef("Regla5_Cotitular" + str(cBP)),
       name(
          nType("M"),
          fullName(str(cfullName)),
       ),       
         roles(
          rCode("ROL_REGLA_5_COTITULAR"),
          rstat("C"),
       ),      
        number(
          nuType("BP"),
...
if __name__ == "__main__":
    with open(path + 'Regla_5_Cotitular.csv') as csv_file:
...
# 		print(mensaje,type(mensaje))
                with open(path2 + 'Regla_5_Cotitular.UMF', 'a+') as archivo:
